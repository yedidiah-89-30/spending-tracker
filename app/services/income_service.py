...
class IncomeService:
    ...
    def get_stats(self, user_id: int, month: int, year: int) -> IncomeStats:
        """Powers GET /api/v1/income/stats. `growth_percentage` compares
        `month`/`year` against the immediately preceding calendar month
        (handling the year rollover when month=1 -> December of year-1)."""
        total_income = Decimal(self.income_repository.sum_total(user_id))
        monthly_income = Decimal(self.income_repository.sum_for_month(user_id, year, month))
        yearly_income = Decimal(self.income_repository.sum_for_year(user_id, year))
        ...
        previous_month_income = Decimal(
            self.income_repository.sum_for_month(user_id, prev_year, prev_month)
        )

        growth_percentage: Decimal | None
        if previous_month_income == 0:
            # Can't compute a meaningful percentage change from a zero
            # baseline - None (not 0, and not a divide-by-zero) tells the
            # frontend there's nothing to compare against yet.
            growth_percentage = None
        else:
            growth_percentage = (
                (monthly_income - previous_month_income) / previous_month_income * Decimal("100")
