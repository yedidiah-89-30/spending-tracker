...
Sprint 3 update: Income is now wired in (total_income, and income rows
feed recent_transactions). Expenses, Savings Goals, and Subscriptions
still don't exist (Sprints 4, 6, 7), so those stay at 0/empty for now.
...
_PENDING_FEATURES = ["expenses", "savings_goals", "subscriptions"]
...
class DashboardService:
    ...
    def get_summary(self, user: User, month: int, year: int) -> DashboardSummary:
        """`month`/`year` scope the totals (total_income, net_profit_loss).
        `recent_transactions` deliberately does NOT scope to that period -
        it's a "latest activity" feed, so it always shows the user's most
        recent entries regardless of which month they're currently viewing
        totals for. This matches how most finance dashboards behave: you
        can look at July's totals while still seeing that you logged
        something yesterday.
        """
        ...
        # Recent transactions should read newest-first across every feature
        # that contributes to it, not just be a flat concatenation.
